// First load the application's bootstrapping procedures / dependencies:
require('./bootstrap');

// Create namespace for the application:
window.NicholasMorgan = {};
...
;(function(app, $, mousetrap, undefined) {
    ...
    app.init = function(config) {
        ...
        // Mousetrap events:
        // Login:
        mousetrap.bind('l o g i n enter', function() {
    ...
    app.jsonStringify = function(unformattedJson, linebreaks = false) {
        if(linebreaks) {
            let stringified = JSON.stringify(unformattedJson, undefined, "\t");
        }
        else {
            let stringified = JSON.stringify(unformattedJson);
        }
        return stringified;
    };

})(window.NicholasMorgan, window.jQuery, window.Mousetrap);

// Load all modules that could possibly be used in the application.
require('./modules/Admin_Home');
